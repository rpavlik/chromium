...
class CR:
	...
	def do_connectrequest( self, sock, args ):
		...
		if (protocol == 'tcpip' or protocol == 'udptcpip'):
			...
			(p, hostname, port_str, node_id_str, port_num_str, endianness_str, lid1, lid2, qp_ous, qp) = connect_info
			...
			for server_sock in self.wrappers.values():
				if server_sock.ib_accept_wait != None:
					(server_hostname, server_port, server_node_id, server_port_num, server_endianness, server_lid1, server_lid2, server_qp_ous, server_qp) = server_sock.ib_accept_wait
					if SameHost(server_hostname, hostname) and server_port == port:
						sock.Success( "%d %d %d %d %s %s %s %s" % (self.conn_id, server_node_id, server_port_num, server_endianness, server_lid1, server_lid2, server_qp_ous, server_qp ) )
						server_sock.Success( "%d %d %d %s %s %s %s" % (self.conn_id, node_id, port_num, lid1, lid2, qp_ous, qp ) )
						self.conn_id += 1
						return
					else:
						CRDebug( "not connecting to \"%s:%d\" (!= \"%s:%d\")" % (server_hostname, server_port, hostname, port) )
			sock.ib_connect_wait = (hostname, port, node_id, port_num, endianness, lid1, lid2, qp_ous, qp)
		elif (protocol == 'gm'):
	...
	def do_acceptrequest( self, sock, args ):
		...
		if protocol == 'tcpip' or protocol == 'udptcpip':
			...
			(p, hostname, port_str, node_id_str, port_num_str, endianness_str, lid1, lid2, qp_ous, qp) = accept_info
			...
			for client_sock in self.wrappers.values():
				if client_sock.ib_connect_wait != None:
					(client_hostname, client_port, client_node_id, client_port_num,
					 client_endianness, client_lid1, client_lid2, client_qp_ous, client_qp) = client_sock.ib_connect_wait
					if SameHost(client_hostname, hostname) and client_port == port:
						sock.Success( "%d %d %d %s %s %s %s" % (self.conn_id, client_node_id, client_port_num, client_lid1, client_lid2, client_qp_ous, client_qp ) )
						client_sock.Success( "%d %d %d %d %s %s %s %s" % (self.conn_id, node_id, port_num, endianness, lid1, lid2, qp_ous, qp ) )
						self.conn_id += 1
						return
					else:
						CRDebug( "not accepting from \"%s:%d\" (!= \"%s:%d\")" % (client_hostname, client_port, hostname, port ) )
			...
			sock.ib_accept_wait = (hostname, port, node_id, port_num, endianness, lid1, lid2, qp_ous, qp)
		elif protocol == 'gm':
