...
void crStateExtensionsInit( CRContext *g )
{
	...
	*/
	...
	if (hasExtension(g->limits.extensions, "GL_ARB_imaging"))
		g->extensions.ARB_imaging = GL_TRUE;

	if (hasExtension(g->limits.extensions, "GL_ARB_multitexture"))
		g->extensions.ARB_multitexture = GL_TRUE;

	if (hasExtension(g->limits.extensions, "GL_ARB_texture_cube_map") ||
		hasExtension(g->limits.extensions, "GL_EXT_texture_cube_map"))
		g->extensions.ARB_texture_cube_map = GL_TRUE;

	if (hasExtension(g->limits.extensions, "GL_EXT_blend_color"))
		g->extensions.EXT_blend_color= GL_TRUE;

	if (hasExtension(g->limits.extensions, "GL_EXT_blend_minmax"))
		g->extensions.EXT_blend_minmax = GL_TRUE;

	if (hasExtension(g->limits.extensions, "GL_EXT_blend_subtract"))
		g->extensions.EXT_blend_subtract = GL_TRUE;

	if (hasExtension(g->limits.extensions, "GL_EXT_texture_edge_clamp"))
		g->extensions.EXT_texture_edge_clamp = GL_TRUE;

	if (hasExtension(g->limits.extensions, "GL_EXT_texture_filter_anisotropic"))
		g->extensions.EXT_texture_filter_anisotropic = GL_TRUE;

	if (hasExtension(g->limits.extensions, "GL_NV_fog_distance"))
		g->extensions.NV_fog_distance = GL_TRUE;

	if (hasExtension(g->limits.extensions, "GL_NV_texgen_reflection"))
