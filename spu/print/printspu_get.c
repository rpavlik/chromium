...
{
	fprintf( print_spu.fp, "GetIntegerv( %s, 0x%x )\n", printspuEnumToStr( pname ), params );
	fflush( print_spu.fp );
...
{
	fprintf( print_spu.fp, "GetFloatv( %s, 0x%x )\n", printspuEnumToStr( pname ), params );
	fflush( print_spu.fp );
...
{
	fprintf( print_spu.fp, "GetDoublev( %s, 0x%x )\n", printspuEnumToStr( pname ), params );
	fflush( print_spu.fp );
...
{
	fprintf( print_spu.fp, "GetBooleanv( %s, 0x%x )\n", printspuEnumToStr( pname ), params );
	fflush( print_spu.fp );
