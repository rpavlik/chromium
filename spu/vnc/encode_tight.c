...
#if RASTER_BOTTOM_TO_TOP
  fb_ptr = g_framebuffer + (g_fb_height - (r->y + r->h)) * g_fb_width + r->x;
#else
...
#if RASTER_BOTTOM_TO_TOP
      pix = g_framebuffer[(g_fb_height - 1 - (r->y + y + d)) * g_fb_width + (r->x + x + d)];
#else
...
#if RASTER_BOTTOM_TO_TOP
  assert(x + count <= g_fb_width);
  fb_ptr = g_framebuffer + (g_fb_height - 1 - y) * g_fb_width + x;
#else
