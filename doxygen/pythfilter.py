...
#             (thanks to Richard Laager for the patch)
# 11.12.2003: Corrected #includes to use os.pathsep instead of ".". Corrected
...
#    sys.stderr.write("%d: %s\n"%(state, tok))
    ...
    # IMPORT_OP
        ...
        if (tok=="."):
            ...
            for m in modules:
                output('#include "'+m.replace('.',os.pathsep)+'.py"\n', import_spos, immediate=1)
...
def convert(srcpath, destpath):
    ...
    for srcfile in sfiles:
        ...
        if ext.lower()!=".py" or ext.lower() != ".conf":
            continue
